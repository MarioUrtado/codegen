...
declare function local:get_RefreshCacheREQ($category as xs:string) as element() (:: schema-element(ns1:LazyRefreshREQ) ::) {
 
  <ns1:LazyRefreshREQ>
        <ns1:Key source="ConversationManager" category="{fn:data($category)}">
        </ns1:Key>
    </ns1:LazyRefreshREQ>
};

local:get_RefreshCacheREQ($category)
