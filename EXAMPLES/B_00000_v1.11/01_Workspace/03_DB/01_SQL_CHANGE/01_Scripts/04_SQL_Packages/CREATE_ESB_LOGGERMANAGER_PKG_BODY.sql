create or replace 
PACKAGE BODY ESB_LOGGERMANAGER_PKG AS
 ...
 PROCEDURE REGISTER_MESSAGE_TRANSACTION(p_CAPABILITY_ID IN NUMBER, p_PROC_ID IN VARCHAR2, p_EVENT_ID IN VARCHAR2, p_CLIENT_REQ_TIMESTAMP IN VARCHAR2, MESSAGE_TRANSACTION_ID OUT NUMBER) IS
  ...
  BEGIN
    ...
    -- a ellos.
    SELECT 
      M.ID INTO MESSAGE_TRANSACTION_ID 
    FROM 
      ESB_MESSAGE_TRANSACTION M 
    WHERE 
      M.PROC_ID = p_PROC_ID AND 
      M.EVENT_ID = p_EVENT_ID AND
    ...
    WHEN NO_DATA_FOUND THEN
      ...
      VALUES (
        ...
        0, -- Se asume que es la primera Tx, y que el MessageManager eligio no registrarla sincronicamente.
        1) 
      RETURNING "ID" INTO MESSAGE_TRANSACTION_ID;
 ...
 END REGISTER_MESSAGE_TRANSACTION;
  ...
  BEGIN
    SELECT ID INTO CHANNEL_ID FROM ESB_CHANNEL WHERE NAME = p_CHANNEL_NAME;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      INSERT INTO ESB_CHANNEL(ID, CODE, NAME, RCD_STATUS) VALUES(ESB_CHANNEL_SEQ.NEXTVAL, '99', p_CHANNEL_NAME, 0) RETURNING "ID" INTO CHANNEL_ID;
  END GET_CHANNEL;
  ...
  BEGIN
    ...
    -- Retorna el CAPABILITY_ID según el ServiceName y ServiceOperation informado. Si no existe la capacidad configurada
    ...
    REGISTER_MESSAGE_TRANSACTION(v_CAPABILITY_ID, v_PROC_ID, v_EVENT_ID, v_CLIENT_REQ_TIMESTAMP, v_TRANSACTION_ID);
    
    -- Estos 3 Procedimientos se aseguran de que exista un País, un Canal y un Consumidor configurados en el FW según los datos
