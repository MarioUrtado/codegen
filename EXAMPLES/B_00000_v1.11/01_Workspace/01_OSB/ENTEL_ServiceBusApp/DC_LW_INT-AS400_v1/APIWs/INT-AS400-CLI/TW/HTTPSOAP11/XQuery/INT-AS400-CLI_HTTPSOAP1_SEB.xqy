...
declare function local:INT-AS400-CLI_HTTPSOAP11_SEB($LegacyResponse as element(), $LegacyResult as element()?) as element() (:: schema-element(ns1:SourceError) ::) {
    
    let $SourceErrorCode	          := local:getSourceErrorCode()
    let $SourceErrorDescription 	  := local:getSourceErrorDescription()
    let $ErrorSourceCode                  := "INT-AS400"
    let $ErrorSourceDetails               := "INT-AS400-CLI_HTTPSOAP11_TW"
...
local:INT-AS400-CLI_HTTPSOAP11_SEB($LegacyResponse, $LegacyResult)
