...
declare function local:CHL-POR-NOD-POR-NOD_HTTPSOAP12_SEB($LegacyResponse as element(), $LegacyResult as element()?) as element() (:: schema-element(ns1:SourceError) ::) {
  
    let $SourceErrorCode	          := local:getSourceErrorCode()
    let $SourceErrorDescription 	  := local:getSourceErrorDescription()
    let $ErrorSourceCode                  := "CHL-POR-NOD"
    let $ErrorSourceDetails               := "CHL-POR-NOD-POR-NOD_HTTPSOAP12_TW"
...
local:CHL-POR-NOD-POR-NOD_HTTPSOAP12_SEB($LegacyResponse, $LegacyResult)
