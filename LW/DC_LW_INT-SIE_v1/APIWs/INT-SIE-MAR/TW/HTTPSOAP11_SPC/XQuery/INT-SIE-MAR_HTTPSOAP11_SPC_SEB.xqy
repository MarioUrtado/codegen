...
declare function local:INT-SIE-MAR_HTTPSOAP11_SPC_SEB($LegacyResponse as element(), $LegacyResult as element()?) as element() (:: schema-element(ns1:SourceError) ::) {
  
    let $SourceErrorCode	          := local:getSourceErrorCode()
    let $SourceErrorDescription 	  := local:getSourceErrorDescription()
    let $ErrorSourceCode                  := "INT-SIE"
    let $ErrorSourceDetails               := "INT-SIE-MAR_HTTPSOAP11_SPC_TW"
...
local:INT-SIE-MAR_HTTPSOAP11_SPC_SEB($LegacyResponse, $LegacyResult)
